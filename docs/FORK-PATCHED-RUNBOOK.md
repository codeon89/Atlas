...
2. Write the merged nightly number into `UPSTREAM_NIGHTLY` — the highest
   `v{base}-nightly.N` tag reachable from the merge (cross-check against
   upstream's Releases page). The workflow fails closed on a stale file, so
   this step is what keeps the new release truthful.
